...
- Verify the board powers up
- Check `lsusb`
- Check `dmesg`

### 19.2 No `/dev/iwr_cli` and `/dev/iwr_data`

- Recheck udev rules
- Reload udev rules
- Unplug/replug the board

### 19.3 Commands only echo back

- Ensure commands end in `\r\n`
- Add startup delay before sending config
- power cycle after a prior crash

### 19.4 Commands show invalid usage

- Config syntax does not match running firmware

### 19.5 Sensor starts but no objects appear

- Confirm raw data bytes are flowing
- verify parser alignment
- confirm correct header format
- inspect `guiMonitor` settings
- ensure target is inside range and field of view

### 19.6 Browser view shows wrong scaling

- Check whether raw integer values are being plotted instead of engineering units
- Prefer plotting `x` vs `range`
- lock display axes to meter ranges

---

## 20. Security and Reliability Notes

- Avoid using `sudo` for routine serial access once `dialout` permissions are configured
- Keep stable symlink names so service scripts do not depend on dynamic `ttyACM` numbering
- Handle serial startup, power cycling, and malformed frames gracefully in software
- Validate every config command before assuming the radar is running

---

## 21. Suggested Next Steps

Once the radar visualization is stable, the next logical stage is cloud integration.

A practical path is:

1. Parse detections into JSON
2. Publish them over MQTT from the Pi
3. Store them in a broker or cloud IoT service
4. Build a remote dashboard for occupancy/motion analytics
...
- per-object range
- per-object Doppler
- per-object lateral position
- per-object azimuth

---

## 22. Summary of Final Working State

The final working setup achieved the following:

- Raspberry Pi successfully communicated with IWR1443BOOST over USB CDC-ACM
- Stable device names were assigned using udev
- The radar accepted the older xWR14xx demo configuration syntax
- The sensor started correctly and streamed binary data frames
- The parser decoded object information and range values
- A real-time browser visualization displayed radar detections in a map-like format

This forms a solid foundation for extending the project into cloud-connected radar sensing.
